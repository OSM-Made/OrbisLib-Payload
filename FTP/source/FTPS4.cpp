...
static void cmd_PORT_func(ftps4_client_info_t *client)
{
	...
	printf("PORT connection to client's IP: %s Port: %d\n", ip_str, data_port);
	...
	printf("Client %i data socket fd: %d\n", client->num, client->data_sockfd);
		...
		while (dent != dend) {
			if (dent->d_name[0] != '\0') {
				...
				if (err == 0) {
					...
					//printf("%s stat returned %d\n", full_path, errno);
				}
			}
			else {
				printf("got empty dent\n");
				goto exit;
			}
...
static void *client_thread(void *arg)
{
	...
	while (1) {
		...
		if (client->n_recv > 0) {
			printf("Received %i bytes from client number %i:\n", client->n_recv, client->num);
	...
	printf("Client thread %i exiting!\n", client->num);
...
static void *server_thread(void *arg)
{
	...
	while (1) {
		...
		if (client_sockfd >= 0) {
			printf("New connection, client fd: 0x%08X\n", client_sockfd);
			...
			printf("Client %i thread UID: 0x%08X\n", number_clients, client->thid);
			...
			printf("Server socket closed, 0x%08X\n", client_sockfd);
			break;
		}
	}
...
int ftps4_init(const char *ip, unsigned short int port)
{
	...
	printf("Client list mutex UID: 0x%08X\n", client_list_mtx);
	...
	printf("Server thread UID: 0x%08X\n", server_thid);
